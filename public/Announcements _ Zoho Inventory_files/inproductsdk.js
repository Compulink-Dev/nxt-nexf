...
initialize(){
    ...
    this.timer=setInterval(function(){ handle.iframe.setAttribute("src",handle.urlsource+"/Notification?ORGID="+handle.orgID+"&ServiceID="+handle.serviceID+"&polling=true&frameorigin="+handle.domain +"&Feedback="+handle.version); },this.failTimeout);
    setTimeout(function(){ handle.iframe.setAttribute("src",handle.urlsource+"/Notification?ORGID="+handle.orgID+"&ServiceID="+handle.serviceID+"&polling=true&frameorigin="+handle.domain +"&Feedback="+handle.version); },delay);
}
...
testtip(){
    ...
    window.addEventListener("message", function(event,a,b){
        ...
        if(event.data.emittype == 'bannerMsgFromWms'){
            ...
            handle.pollingenabled ? '' : handle.iframe.contentWindow.postMessage({ "emittype": "Bannerwmscallback" }, handle.url);
        }
        if(event.data.emittype == 'bannerServerupFromWms'){
            handle.wmsenabled = event.data.serverup;
            handle.iframe.contentWindow.postMessage({ "emittype": "Bannerwmscallbackserver", "serverup": event.data.serverup }, handle.url);
        }
    });
	...
	var tipCheckBackend = function(){
        if(handle.promotionID===sessionStorage.getItem("clickedNotificationID"+handle.serviceID)){
            ...
            handle.iframe.contentWindow.postMessage( {"type":"tabSwitch","promotionID":handle.promotionID,"feedback":0} ,handle.url);
        }
	}
...
drawTip(data){
    ...
    if(this.pid===localStorage.getItem("micsnotificationid"+handle.serviceID)){
        handle.iframe.contentWindow.postMessage( {"promotionID":handle.pid,"feedback":4} ,handle.url);	 //No I18n
    }
    if(this.pid===localStorage.getItem("micsnotificationid"+handle.serviceID)||handle.tipDiv.innerHTML!==""||!tipData.type){
        return;
    }
    this.iframe.contentWindow.postMessage({"promotionID":this.pid,"feedback":2},this.url);  //No I18n
    ...
    var addMICSbannerClickListener = function (selector, callback) {
	    ...
	    if(linkType!='walkthrough' && linkType!='web url')
            ...
            if(linkType=='walkthrough'){ 
                ...
                element.addEventListener('click',()=>{WalkthroughSDK.getInstance().triggerFlow(flowID,"banner",handle.promotionID);
                handle.promotionClicked(); })
            }
            else{
...
drawBanner(data,par,pid){
    ...
    toggle.addEventListener("click",function(e){
  	  ...
  	  close.addEventListener("click",function(){
            ...
            handle.iframe.contentWindow.postMessage( {"promotionID":pid,"feedback":4},handle.url);  //No I18n
    ...
    function loadiframe(){
        if(data.iframeSrc){
            iframe.addEventListener("load",function(){   
                ...
                iframe.contentWindow.postMessage( {"type":"userData","zuid":handle.zuid,"orgID":handle.orgID,"userData":handle.userData} ,data.iframeSrc);
			});
            if(data.iframesdk){
    ...
    img.addEventListener('load', function() {    loadiframe();    });
      ...
      Array.prototype.forEach.call(content.getElementsByClassName("micsSameLink"),function(d){ //No I18n
        d.addEventListener("click",function(){
            handle.iframe.contentWindow.postMessage({"promotionID":pid,"feedback":3},handle.url); //No I18n
...
promotionClicked(){
    var handle = this;
    handle.iframe.contentWindow.postMessage({"promotionID":this.promotionID,"feedback":3},handle.url); //No I18n
    if(handle.wmsenabled){
        handle.pollingenabled = true;
        handle.iframe.contentWindow.postMessage({ "emittype": "Bannerwmspoll"},handle.url);
    }
...
promotionClosed(){
    var handle = this;
    handle.iframe.contentWindow.postMessage({"promotionID":this.promotionID,"feedback":4},handle.url); //No I18n
    if(handle.wmsenabled){
        handle.pollingenabled = true;
        handle.iframe.contentWindow.postMessage({ "emittype": "Bannerwmspoll"},handle.url);
    }
        ...
        switch(d.type){
            ...
            if(d.check){
                ...
                if(d.flowID){
                    ...
                    celem.addEventListener('click',()=>{WalkthroughSDK.getInstance().triggerFlow(d.flowID,"banner",handle.promotionID)})
                }  
                else{
...
class walkthrough{
    ...
    getselfhelpmetadata(){
        return [ { url:"micsz/Solopreneur/*/mobilenotification", checktype:"urlpatternmatch", selfhelpdata:[ { selfhelpId:12312,datalist:[ {"type":"walkthrough", id:12345, "displayname":"page one"}, {type:"video",id:12346,"displayname":"Register Zoholics"}] }, {  selfhelpId:123,datalist:[ {"type":"walkthrough", id:1245, "displayname":"Create dashboard"}, {type:"video",id:1236,"displayname":"Zoholics banner"}] } ] } , { url:"micsz/mics/engagement", checktype:"urlpatternmatch", selfhelpdata:[ {selfhelpId:12312,datalist:[ {"type":"walkthrough", id:12345, "displayname":"page two"}, {type:"video",id:12346,"displayname":"Register Zoholics"}] }, {selfhelpId:123,datalist:[ {"type":"walkthrough", id:1245, "displayname":"Create dashboard"}, {type:"video",id:1236,"displayname":"Zoholics banner"}] } ] }  , { url:"*", checktype:"urlpatternmatch", selfhelpdata:[ {selfhelpId:12312,datalist:[ {"type":"walkthrough", id:12800, "displayname":"Mics Tour"}, {type:"Walkthrough",id:12820,"displayname":"Explore Mics Dashboard"}] }, {selfhelpId:123,datalist:[ {"type":"walkthrough", id:12800, "displayname":"Test Mics tabs for selfhelp"}, {type:"Walkthrough",id:12820,"displayname":"New Customized views in MICS"}] } ] }  , { url:"micsz/mics/engagement", checktype:"urlpatternmatch", selfhelpdata:[ {selfhelpId:12312,datalist:[ {"type":"walkthrough", id:12345, "displayname":"page two copy"}, {type:"video",id:12346,"displayname":"Register Zoholics"}] }, {selfhelpId:123,datalist:[ {"type":"walkthrough", id:1245, "displayname":"Create dashboard"}, {type:"video",id:1236,"displayname":"Zoholics banner"}] } ] } ]
    }
  }
